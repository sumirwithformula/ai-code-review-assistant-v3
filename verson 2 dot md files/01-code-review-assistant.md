...
**Stack:** Python, **Gemini API** (free tier via Google AI Studio — no card
needed), `flake8`/`pylint` (optional but recommended), optional GitHub API
for pulling real PR diffs.

> **Why Gemini for now:** you're using this while your Anthropic API billing
> gets sorted out. Google AI Studio gives a genuinely free tier (no card
> required) at ai.google.dev. The code below is structured so swapping back
> to the Claude API later is a ~5-line change — see the note at the bottom.
...
```
 ...
 └─────────┬──────────┘
           ...
           ▼
 ┌────────────────────┐
 │   LLM Reviewer      │  Gemini API call, structured-output prompt
 └─────────┬──────────┘
...
```bash
pip install google-genai flake8
```
...
Gemini supports forcing valid JSON directly via `response_mime_type` and a
`response_schema`, which is more reliable than asking nicely in the prompt.
...
PROMPT = """You are a senior code reviewer. Review this diff for bugs,
...
def review_diff(diff_text: str) -> dict:
    resp = client.models.generate_content(
        ...
        contents=PROMPT.format(diff_text=diff_text),
...
```
Tip: the `response_schema` constraint means you don't need defensive
JSON-parsing/retry logic here — Gemini will conform to the schema directly.
...
- Latency/cost per review call.

### Resume bullet template
> Built an AI-assisted code review tool combining static analysis (flake8)
> with an LLM-based structured-output reviewer to implement an automated
> quality gate; tested on [N] diffs, catching [X/Y] intentionally injected
> defects with ~[Z]s latency per review.

---

## Swapping back to Claude later
Once your Anthropic billing is sorted, only `review_diff()` changes:
```python
...
def review_diff(diff_text: str) -> dict:
    resp = client.messages.create(
        ...
        messages=[{"role": "user", "content": PROMPT.format(diff_text=diff_text) +
            "\n\nReturn ONLY valid JSON matching this shape: " + json.dumps(REVIEW_SCHEMA)}],
    )
    return json.loads(resp.content[0].text.strip())
```
Everything else — the flake8 pass, the quality gate, the CLI/FastAPI
wrapper, the tests — is provider-agnostic and doesn't need to change. If
your resume/interview story specifically calls for "Claude API," this is
the only function you need to touch before your final demo/measurement run.
